...
#define BLOCK_SB_SIZE     (32*1024)
#define BUFFER_SIZE    (2 * BLOCK_SB_SIZE)
...
static int8_t *block2 = &(DMA_Buffer[BLOCK_SB_SIZE]);
...
int8_t play_music(int8_t* filename){
    ...
    if (read_dentry_by_name((uint8_t*)filename, &audio_dentry) == -1){
        ...
        audio_file_inode = audio_dentry.inode_num;
    }
    uint8_t magic[4];
    ...
    _fd = fd;
    uint32_t bytes_read = read_data(audio_file_inode,current_offset, (uint8_t*)DMA_Buffer, BLOCK_SB_SIZE*2);
    Transfer_Sound_DMA(1, 0x48 | 0x10, (uint32_t)(&DMA_Buffer[0]), sizeof(DMA_Buffer));
    Set_Sample_Rate(800);
    start_play(BLOCK_SB_SIZE);
    is_playing = 1;
...
void sb16_handler(){
    ...
    cli();
    //printf("Offset: %d\n", current_offset);
    uint32_t bytes_read = read_data(audio_file_inode, current_offset, (uint8_t*)cur_block, BLOCK_SB_SIZE);
    current_offset += bytes_read;
    //printf("Bytes: %d\n", bytes_read);
    //printf("Offset: %d\n", current_offset);
    sti();

    if(bytes_read == BLOCK_SB_SIZE){
